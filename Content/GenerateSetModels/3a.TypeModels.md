...
## Species limits
Models that test the existance a given number of species. These models do not require _special_ edits to a `migrationArray` but it does require models with fully resolved trees.  The species hypotheses are specified using the argument `setCollapseZero` in the `GridSearch`function.