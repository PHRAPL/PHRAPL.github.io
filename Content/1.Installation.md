...
**<font color='#006579'>PHRAPL</font> can only be used in macOS and Linux.** <font color='#006579'>PHRAPL</font> requires `R`, `ms`, and `perl` to be installed in your computer. 
...
wget http://cran.nexr.com/src/contrib/rPython_0.0-5.tar.gz
...
To deactivate, activate, save conda environment in `yml` file, then and create a new environment from such a file, [please see above for the macOS version instructions](https://phrapl.github.io/Content/1.Installation.html#activate--deactivate-conda-environment).
...
The recipe to create a conda environment (see above) in meant to avoid any compatibility issues with other R packages, it  should install all the required dependencies.  But if you still have an issue, please make sure that the following packages are working properly.
